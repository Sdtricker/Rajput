...
# ✅ Vercel only allows writing inside /tmp directory
...
def load_data():
    ...
    try:
        ...
        data = {"users": {}, "redeem_codes": {}, "api_key": DEFAULT_API_KEY, "total_searches": 0}
    data.setdefault("total_searches", 0)
    data.setdefault("api_key", os.environ.get('LEAKOSINT_API_KEY', DEFAULT_API_KEY))
    return data
...
def serve_static(filename):
    if filename.endswith(('.png', '.jpg', '.jpeg', '.html')):
...
# ✅ No app.run() needed — Vercel automatically detects and runs the app
